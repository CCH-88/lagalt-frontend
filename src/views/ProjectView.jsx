...
function ProjectView() {
	...
	const tryJoinProject = async () => {
		...
		const [checkError, projectResponse] = await joinProject(projectId, keycloak.subject, keycloak.token)
		if (checkError !== null) {
			setApiError(checkError)
		...
		getMembers() //attempt at a reactive update
		setLoading(false)
	}
	/*
  const tryJoinProject = async () => {
			...
			{project !== null && (
				<>
					...
					<div className={styles.projectCard}>
						<button className="bg-gray-200 hover:bg-gray-300 duration-300 rounded-full px-4 py-2 my-2 font-light text-sm mx-auto" onClick={forceUpdate}>
							Update chat
						</button>
						...
						{showJoin ? (
							<form className="w-full flex flex-col justify-center">
								...
								<textarea
									className="mx-4 border my-2  h-12 flex-wrap px-2"
									type="text"
									autoComplete="off"
									placeholder="Enter a name for your project here"
									{...register('motivation', { required: true, minLength: 3 })}
								/>

								<button className="bg-gray-200 hover:bg-gray-300 duration-300 rounded-full px-4 py-2 my-2 font-light text-sm mx-auto" onClick={handleSubmit(tryJoinProject)}>
									Join project
								</button>
							</form>
						) : (
							<button className="bg-gray-200 hover:bg-gray-300 duration-300 rounded-full px-4 py-2 my-2 font-light text-sm mx-auto" onClick={() => setShowJoin(true)}>
